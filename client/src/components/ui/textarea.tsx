...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(({ className, error, "aria-describedby": ariaDescribedby, ...props }, ref) => {
  return (
    <textarea
      className={cn(
      ...
      ref={ref}
      aria-invalid={error}
      aria-describedby={error ? `${ariaDescribedby || ""} error-message` : ariaDescribedby}
      {...props}
    />
  );
});
